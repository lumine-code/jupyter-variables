...
class VariablesSession {
  ...
  /**
   * Show a name in jupyter-explorer, if that package is installed.
  ...
  /**
   * Whether the Variables panel is on screen. A closed panel has no reason to
   * keep re-reading namespaces, so the stores drop their idle subscriptions
   * while nothing is looking and pick them back up — flags intact — when the
   * panel reopens.
  ...
  setViewActive(active) {
    ...
    for (const store of this.stores.values()) {
      store.setActive(this.viewActive);
    }
  }
  ...
  storeFor(kernel = this.kernel) {
    ...
    if (!this.stores.has(kernel)) {
      this.stores.set(kernel, new VariablesStore(kernel, this.viewActive));
    }
